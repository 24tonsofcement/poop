...
func _ready() -> void:
	...
	for state in ["normal", "hover", "pressed", "disabled"]:
		add_theme_stylebox_override(state, preload("res://game/graphic_skin.gd").panel(Color("faf8f1"), Color("212322"), 0))
...
func draw_ornament() -> void:
	...
	ornament.draw_rect(Rect2(5, 5, 30, 25), Color("d43922"))
	ornament.draw_string(get_theme_default_font(), Vector2(9, 23), "04", HORIZONTAL_ALIGNMENT_LEFT, -1, 16, Color.WHITE)
	ornament.draw_rect(Rect2(w - 85, 8, 77, 21), Color("f9d44b"))
	ornament.draw_string(get_theme_default_font(), Vector2(w - 80, 23), "ON DECK", HORIZONTAL_ALIGNMENT_LEFT, -1, 12, ink)
	var cursor: float = (sin(clock * 1.6) + 1.0) * 0.5
	ornament.draw_rect(Rect2(4, h - 5, w - 8, 4), Color("d43922"))
